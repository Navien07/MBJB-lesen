...
| 10 | **M6 finding: visual glyph-height estimation by a multimodal model does NOT meet ±0.05.** Live check on 2026-07-29 (claude-opus-4-8, clean SVG-generated boards): board-070 measured 0.861 vs truth 0.70 (Δ0.161); board-086 measured 1.188 vs truth 0.86 (Δ0.328) — both at self-reported confidence ≈0.94, i.e. confidently wrong. The low-res board escalated correctly. Per BUILD-PLAN the tolerance was NOT widened; the input contract changed instead: **signboard artwork must be an annotated production proof** with per-run lettering heights printed beside each run (industry-standard signage proofs carry these). The agent reads the printed annotations (`measurement_basis: "annotation"`) and escalates anything unannotated or estimated. MBJB should confirm it can require annotated proofs at submission; if not, per-run lettering heights must become declared form fields instead. | The constraint MBJB needs to know before Phase 1; affects `SIGN-SIZE-002`, `SIGN-SIZE-001`, `SIGN-DIM-001` inputs |
